...
/// The four localized strings of the recovery dialog.
#[derive(Debug)]
...
mod tests {
    ...
    fn roundtrips_through_a_pool() {
        ...
        let cmd = UpdateLabels::new(&pool, "Recovery", "frogans*…", "Open", "Cancel");
        let back = UpdateLabels::from_raw(cmd.to_raw(), &pool.as_pool());
        assert_eq!(back.title.unwrap().as_str().unwrap(), "Recovery");
